...
class MinimalPublisher(Node):
    ...
    def __init__(self, name):
        ...
        self.publisher_ = self.create_publisher(Twist, f'model//cmd_vel', 10)
        self._logger.info(f"__init__ {name}")
    ...
    def foo(self):
        while True:
            time.sleep(2)
            msg = Twist()
            msg.linear.x = float(random.randrange([-1, 1]))
            msg.angular.z = float(random.randrange([-1, 1]))
